  ...
  <div>
    ...
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column label="情绪评分">
        <template #default="scope">
          <el-rate v-model="scope.row.moodScore" :show-text="true" :max="10" disabled />
        </template>
      </el-table-column>
      ...
      <el-table-column label="情绪触发因素" width="120" prop="emotionTriggers"></el-table-column>
      <el-table-column label="情绪日记内容" width="200" prop="diaryContent" />
      <el-table-column label="操作" width="180" fixed="right">
        <template #default="scope">
          <el-button @click="viewSesstionDetail(scope.row)" type="primary" text>详情</el-button>
          <el-button @click="deleteSesstion(scope.row)" type="danger" text>删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页 -->
    <el-pagination v-model:current-page="pagination.currentPage" v-model:page-size="pagination.pageSize"
      layout="prev, pager, next, total" :total="pagination.total" @current-change="handleCurrentChange" />

    <el-dialog v-model="detailDialogVisible" title="情绪详情" width="800px" :close-on-click-modal="false">
        ...
        <div class="detail-section">
          ...
          <el-descriptions :column="2" border>
            <el-descriptions-item label="情绪评分">
              <el-rate v-model="currentDetail.moodScore" :show-text="true" :max="10" disabled />
            </el-descriptions-item>
            <el-descriptions-item label="主要情绪">
              <el-tag :type="getAiEmotionTagType(currentDetail.dominantEmotion)">{{ currentDetail.dominantEmotion || '无'
              }}</el-tag>

            </el-descriptions-item>
            <el-descriptions-item label="睡眠质量">{{ currentDetail.sleepQuality || '无' }}/5</el-descriptions-item>
            <el-descriptions-item label="压力等级">{{ currentDetail.stressLevel || '无' }}/5</el-descriptions-item>
          </el-descriptions>
        </div>
        <div class="detail-section">
          <h4>情绪触发因素</h4>
          <el-descriptions :column="2" border>
            ...
            <el-descriptions-item label="情绪日记内容">{{ currentDetail.diaryContent || '无' }}</el-descriptions-item>
          </el-descriptions>
        </div>
        <div class="detail-section">
          <h4>Ai分析结果</h4>
          <div class="ai-analysis-result">
            ...
            <el-descriptions :column="2" border>
              ...
              <el-descriptions-item label="情绪性质">
                <el-tag :type="aidata.isPositive ? 'success' : 'danger'">{{ aidata.isPositive ? '正面' : '负面' }}</el-tag>
              </el-descriptions-item>
            </el-descriptions>
...
const formItem = [
  { comp: 'input', label: '用户ID', prop: 'userId' },
  {
    comp: 'select', label: '情绪详情', prop: 'moodScoreRange', placeholder: '请选择情绪范围 ', options: [
      { label: '低分(1-3)', value: '1-3' },
      { label: '中性(4-6)', value: '4-6' },
...
const handleSearch = async (formData = {}) => {
  ...
  try {
    const res = await getEmotionalLogs(params)
    if (res.code == 200) {
      tableData.value = res.data.records || []
      pagination.total = res.data.total || 0
    }
  } catch (error) {
    console.error('获取情绪日志失败', error)
  }
}
onMounted(() => {
  handleSearch()
...
//删除
const deleteSesstion = async (row) => {
  ElMessageBox.confirm('确认删除吗？', '提示', {
    ...
    try {
      const res = await deleteEmotionalLog(row.id)
      if (res.code == 200) {
